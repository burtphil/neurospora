...
def make_amp_dict(state,ref_state, p,pref):
    ...
    for idx, valx in enumerate(amp_names):
        ...
        if get_amp(ref_state[:,idx]) != 0:
            rel_amp = rel_difference(get_amp(current_state), get_amp(ref_state[:,idx]))/rel_difference(p,pref)
...
def make_period_dict(state, ref_state, p,pref):
    ...
    for idx, valx in enumerate(per_names):
        ...
        if get_period(ref_state[:,idx]) != 0:
            rel_per = rel_difference(get_period(current_state), get_period(ref_state[:,idx]))/rel_difference(p,pref)
