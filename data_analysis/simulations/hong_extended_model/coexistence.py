...
def coex(T,z0,kappa, strain =rate, ratio = 1, tcycle = 100):
    
    ini = 10 * np.random.rand(7,)
    #warm_dur = T*(1-kappa)
    t      = np.arange(0,tcycle*T,0.1)
    state = odeint(clock,ini,t,args=(strain,T,z0,kappa))
    ...
    ax1.plot(state[lt:,0],state[lt:,1], "k")
    
    save_to = '/home/burt/neurospora/figures/test/'
    fig.savefig(save_to+"ini"+str(ini[0])+".png")
    plt.close(fig)

def zoom(T,z0,kappa, state = state0, strain =rate, ratio = 1, tcycle = 100):
