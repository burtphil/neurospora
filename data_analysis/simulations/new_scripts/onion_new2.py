...
def ztan(t,T,kappa, s=10, z0 = 0.10):
    pi = np.pi
    om = 2*pi/T
...
thermoperiod = np.linspace(0.01,1,100)
zeitgeber_period = np.linspace(25,32,100)
...
for idx, valx in enumerate(thermoperiod):
    for idy, valy in enumerate(zeitgeber_period):
        ...
        state   = odeint(clock,state0,t,args=(rate7,T,kappa))
                  
            # dummy var to define number of tcycles to keep
        lt = int(-(15*10*T))     
            
        x0      = state[lt:,1]
        z_state = ztan(t,T,kappa)
        ...
        c1 = np.abs(T-1*period)*60
        c2 = np.std(np.diff(frq_per))
        
        if c1 < 5 and c2 < 0.5 :
            
            z_minima=np.arange(T/2,cycle_nr*T,T)
...
np.savez("onion_frq7_1_1_z01", warm_mesh = warm_mesh, zeit_mesh = zeit_mesh, ent = ent)
...
plt.xlabel("T [h]")
plt.ylabel(r"$\kappa$ [a.u.]")
plt.show()
fig.savefig("onion_frq7_1_1_z01.pdf", dpi = 1200)
