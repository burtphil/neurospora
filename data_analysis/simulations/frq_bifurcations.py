...
for key in params:
    ...
    bifurcation_dict = rate.copy()

    
    bif_array = np.linspace(bifurcation_dict[key] / 2, bifurcation_dict[key] * 2, 100)
    ...
    for idx, valx in enumerate(bif_array):
        bifurcation_dict[key] = valx
        state = odeint(clock,state0,t,args=(bifurcation_dict,))
        ...
        if (frq_tot_max_array[idx] - frq_tot_min_array[idx] > 1):
    ...
    plt.xlabel(key)
    plt.ylabel("FRQ_tot FLIP, a.u.")
    
    plt.subplot(2,2,3)
    plt.plot(bif_array,period_frq_tot_array,'k')
    plt.xlabel(key)
    plt.ylabel("period, h")
